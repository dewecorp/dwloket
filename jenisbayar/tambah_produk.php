...
if (isset($_POST['tambah_produk'])) {
    ...
    $check_stmt = $koneksi->prepare("SELECT id_produk FROM tb_produk_orderkuota WHERE kode = ?");
    ...
    if ($check_result && $check_result->num_rows > 0) {
        ...
        $insert_stmt = $koneksi->prepare("INSERT INTO tb_produk_orderkuota (kode, keterangan, produk, kategori, harga, status, id_bayar) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $insert_stmt->bind_param("ssssidi", $kode, $keterangan, $produk, $kategori, $harga, $status, $id_bayar);
