...
if (isset($_POST['simpan']) && isset($_POST['jenis'])) {
    ...
    if (empty($jenis)) {
        ...
        $check_query = "SELECT id_bayar FROM tb_jenisbayar WHERE jenis_bayar = ?";
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            $insert_query = "INSERT INTO tb_jenisbayar (jenis_bayar) VALUES (?)";
            $stmt_insert = $koneksi->prepare($insert_query);
            $stmt_insert->bind_param("s", $jenis);
    ...
    if (empty($kode) || empty($produk) || empty($kategori) || $harga <= 0) {
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            $insert_query = "INSERT INTO tb_produk_orderkuota
                            ...
                            VALUES (?, ?, ?, ?, ?, NULL)";
            $stmt_insert = $koneksi->prepare($insert_query);
            $stmt_insert->bind_param("sssdi", $kode, $produk, $kategori, $harga, $status);
...
<html lang="en">
    ...
    <body>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            updateSwalOptions.html = <?= json_encode($update_message, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
            <?php else: ?>
            updateSwalOptions.text = <?= json_encode($update_message, JSON_UNESCAPED_UNICODE) ?>;
            <?php endif; ?>

            Swal.fire(updateSwalOptions);
            <?php endif; ?>
