...
switch ($page) {
    ...
    case 'deposit_history':
        $title = 'Riwayat Deposit OrderKuota';
        $where_conditions = ["1=1"];
        if ($filter_status) {
            $where_conditions[] = "status = '" . mysqli_real_escape_string($koneksi, $filter_status) . "'";
        }
        ...
        $query = "SELECT * FROM orderkuota_deposit WHERE $where_clause ORDER BY tgl DESC, id_deposit DESC";
        $headers = ['No', 'Tanggal', 'Jumlah', 'Metode Pembayaran', 'Status', 'Reference ID', 'Transaction ID', 'Keterangan'];
