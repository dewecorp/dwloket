...
$stmt_check = $koneksi->prepare("SELECT id_produk FROM tb_produk_orderkuota WHERE kode = ?");
$stmt_update = $koneksi->prepare("UPDATE tb_produk_orderkuota
                ...
                WHERE kode = ?");
$stmt_insert = $koneksi->prepare("INSERT INTO tb_produk_orderkuota
...
foreach ($data['value'] as $index => $item) {
    ...
    if ($check_result && $check_result->num_rows > 0) {
        ...
        $stmt_update->bind_param("sssdiss", $keterangan, $produk, $kategori, $harga, $status, $id_bayar, $kode);
        ...
        $stmt_insert->bind_param("ssssisi", $kode, $keterangan, $produk, $kategori, $harga, $status, $id_bayar);
