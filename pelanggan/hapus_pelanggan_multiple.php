...
if (isset($_POST['id_pelanggan']) && is_array($_POST['id_pelanggan'])) {
    foreach ($_POST['id_pelanggan'] as $id_pelanggan) {
        ...
        if ($id_pelanggan > 0) {
            ...
            $stmt_select = $koneksi->prepare("SELECT * FROM pelanggan WHERE id_pelanggan = ?");
            $stmt_select->bind_param("i", $id_pelanggan);
            $stmt_select->execute();
            $sql_data = $stmt_select->get_result();
            ...
            $stmt_delete = $koneksi->prepare("DELETE FROM pelanggan WHERE id_pelanggan = ?");
